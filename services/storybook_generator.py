...
# ==================== 配置（从 config.yaml 加载）====================
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "storybooks")

_cfg = load_config()
_qcfg = _cfg["qwen"]
_ccfg = _cfg["consistency"]

# 供外部模块 import 的常量（保持兼容性）
QWEN_MODELS          = _qcfg["models"]
DEFAULT_QWEN_MODEL   = _qcfg["default_model"]
REF_MODEL            = _qcfg["ref_model"]
DEFAULT_IMAGE_SIZE   = _qcfg["default_image_size"]
STYLE_TEMPLATES      = _cfg["style_templates"]
USE_CHARACTER_PREFIX       = _ccfg["use_character_prefix"]
CHARACTER_CONSISTENCY_MODE = _ccfg["mode"]
FIXED_SEED                 = _ccfg["fixed_seed"]
REFERENCE_NEGATIVE_PROMPT  = _ccfg["reference_negative_prompt"]
...
def _is_wan26_model(model):
    """判断是否为 wan2.6/2.5 同步API模型（向后兼容代理）"""
    from tools.qwen_client import _is_wan26_model as _impl
    return _impl(model)
